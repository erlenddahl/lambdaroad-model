    ...
    class Program
    {
        async static Task Main(string[] args)
        {
            ...
            using (var cip = new ConsoleInformationPanel("Running road network signal loss calculations"))
            {
                ...
                cip?.Set("Tile size", tiles.TileSize);
                cip?.Set("Calculation radius", radius);

                await tiles.Preload(center, radius, cip);

                var road = new RoadNetworkCalculator(tiles, @"..\..\..\..\Data\RoadNetwork\2021-05-28_smaller.shp", radius, center);

                var start = DateTime.Now;
                var calculations = road.Calculate(cip);
                ...
                start = DateTime.Now;
                road.SaveResults(@"..\..\..\..\Data\RoadNetwork\test-results-huge.shp", cip);
